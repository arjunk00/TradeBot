...
if __name__ == "__main__":
    ...
    trainingdatafile = open(
        f"{os.path.dirname(os.path.realpath(__file__))}/trainingdata/processed/{stock_code}__EQ__NSE__NSE__5MINUTE_TRAINING.csv",
        "r")
    ...
    X = np.array(Xlst)
    y = np.array(ylst)
    adanidoublelogit = md.LinearReg(stock_code)
    adanidoublelogit.train(X, y)
    makepickle(adanidoublelogit, f'LINREG_5MIN_TRAINED_{stock_code}')
