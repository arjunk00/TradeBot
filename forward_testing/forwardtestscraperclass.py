...
from stockfunctions import ohlc, store, createsignaltable
    ...
    def run(self):
        ...
        while True:
            if priceticks.pricelistcopy == []:
                ...
                continue
            elif priceticks.pricelistcopy[0] + priceticks.duration <= dt.datetime.now() <= priceticks.pricelistcopy[
                0] + 2 * priceticks.duration:
                ...
                if up_prob > threshold_dict[stock_code]:
                    ...
                    if df[0] > df[3]:
                        ...
                        signal = "S"
                        date_time = datetime.datetime.now()
                tuplelist = (str(date_time.date()), str(date_time.time()), df[0], df[1], df[2], df[3], signal, priceticks.pricelistcopy[-1], up_prob)
